...
def judge(i: int, answer: str) -> list[str]:
    ...
    if re.search(r"official (release|model|qwen)", a) and not re.search(r"not (an? )?(the )?official", a):
...
def main() -> int:
    ...
    try:
        ...
        check("LiteLLM sent them to node-1 llama-swap",
              bool(ours) and all(r["api_base"].rstrip("/") == "http://gx-llama-swap-node01:8080/v1" for r in ours),
              api_base=sorted({r["api_base"] for r in ours}))
        tokens_owui = sorted((t["prompt_tokens"], t["completion_tokens"]) for t in transcript)
        tokens_gw = sorted((r["prompt_tokens"], r["completion_tokens"]) for r in ours)
        check("token counts Open WebUI returned == token counts the gateway recorded", tokens_owui == tokens_gw,
              owui=tokens_owui, gateway=tokens_gw)
        ...
        check("llama-swap node01 proxied them (from the gx-litellm container)",
              len([s for s in swap if "172.21.0.4" in s]) >= len(QUESTIONS) * 2, lines=len(swap))
