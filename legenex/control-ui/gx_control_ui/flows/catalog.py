...
_NODES: list[NodeType] = [
    ...
    _llm_node("ai.llm", "LLM Prompt", "Ask a text model. Connected text is appended to the instruction.",
              ...
              ("chat", "gpt", "ask")),
    _llm_node("ai.vision", "Describe Image",
              "Reads an image with a vision model and writes about it. This is what lets an image reach a "
              "text input: connect a picture here and the answer is text like any other.",
              (_in("image", "Image", "image", required=True), _in("context", "Context", "text", multiple=True)),
              (_out("text", "Description", "text"),),
              (_area("instruction", "What to write", card=True, max_length=4000,
                     placeholder="Describe this image in two sentences."),
               _num("max_tokens", "Max tokens", 512, 16, 4096, integer=True)),
              ("vision", "caption", "describe", "image to text", "ocr")),
