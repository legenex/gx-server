...
#
# GX_CLUSTER_DFLASH2_OVERLAY: backported from upstream vLLM main and
# adapted for jstarkg/vllm-gb10-flashnext:0.28-sm121-r6 (no
# LogitsProcessor.get_top_k_tokens). Do not filter checkpoint tensors.
...
class DFlash2Qwen3ForCausalLM(DFlashQwen3ForCausalLM):
    ...
    def compute_candidates(
        ...
        # GB10 0.28 LogitsProcessor has no get_top_k_tokens; top-k on full logits.
        logits = self.candidate_logits_processor(self.lm_head, hidden_states)
        if logits is None:
            raise RuntimeError("DFlash2 candidate logits are None")
