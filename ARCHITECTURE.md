...
* **Video eviction of music (D-038)** goes only through the supervisor:
  `POST /v1/music/unload {"if_idle": true}`, verified by engine state,
  container and ledger. Each side publishes the memory its running load has
  not taken yet (`/health` `memory.pending_gib`), and the other side
  subtracts it.
...
| media router 2.4 | projected MemAvailable ≥ 30 GiB (growth 57/72 GiB cold, measured warm; keyframe edit refused); gx-music pending subtracted; idle-music eviction; waiting video jobs; holds; pins |
| music supervisor 1.1 | guard admission (32 GiB + 30 GiB reserve + router pending); holds; pins; `if_idle` unload |
...
* **System prompt:** generated by `gx_control_ui/owui_identity.py` from
  `legenex/models/registry.json`.

The facts in the prompt come from each alias's `identity` block and are used
only while that block matches the current binding.

```
registry.json ──► owui_identity (Control Center) ──docker exec──► Open WebUI model layer
      ▲                  ▲            ▲
Model Manager      Setup page     integrity audit (drift = WARN)
(assign/rollback)  (Sync button)
```

Identity is not routing. Routing is proven by:
* the LiteLLM spend log (key hash, `model_group`, `api_base`, token counts);
