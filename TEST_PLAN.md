...
- [ ] **BLOCKED — B-011.** Difficult reasoning/coding inference cannot pass:
  GPU path produces garbage output, isolated to a CUDA/GDN kernel bug in
  this llama.cpp build (`TEST_RESULTS.md` §11). Do not mark this done by
  routing to CPU-only — that violates the "no accidental CPU execution"
  requirement and is not fast enough to be a real tier.
- [x] safe single-node fit (memory behavior fully understood, `RssFile`
  mmap'd, not the failure mode)
- [ ] lifecycle start/stop, safe memory headroom — moot until B-011 is fixed
